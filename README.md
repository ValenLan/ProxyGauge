...
3. 查看“代理状态”和“系统实际出口”；需要时手动刷新以重新核验当前系统网络路径。
...
- macOS 与 Windows 主界面的“系统实际出口”卡遵循当前系统代理、PAC、VPN、TUN 或直连
  ...
  也不持久化出口结果。无代理时，这些 HTTPS 查询会让所用服务看到真实公网 IP 和请求时间，
  但不会发送本机代理端口、订阅、节点、凭据或检测历史。仪表盘处于前台时最多每 5 分钟自动
  核验一次；切回应用、网络路径变化或手动刷新也会立即使旧结果失效并重新检测。
