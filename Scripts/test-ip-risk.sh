...
/usr/bin/grep -Fq '个查询源确认出口一致' "$PROJECT_ROOT/Scripts/puffroute-check.sh"
/usr/bin/grep -Fq '触发 Cloudflare challenge' "$PROJECT_ROOT/Scripts/puffroute-check.sh"
/usr/bin/grep -Fq 'TUN DNS 返回 Fake-IP' "$PROJECT_ROOT/Scripts/puffroute-check.sh"
/usr/bin/grep -Fq '登录账户状态: 未验证' "$PROJECT_ROOT/Scripts/puffroute-check.sh"
/usr/bin/grep -Fq 'Google / Gemini 链式代理' "$PROJECT_ROOT/Scripts/puffroute-check.sh"
/usr/bin/grep -Fq 'https://browserleaks.com/ip' "$PROJECT_ROOT/Scripts/puffroute-check.sh"
/usr/bin/grep -Fq 'https://iphey.com/' "$PROJECT_ROOT/Scripts/puffroute-check.sh"
/usr/bin/grep -Fq 'https://scamalytics.com/ip/$EXT' "$PROJECT_ROOT/Scripts/puffroute-check.sh"
