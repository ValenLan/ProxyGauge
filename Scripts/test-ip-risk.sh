...
output=$(/usr/bin/osascript -l JavaScript \
  "$PROJECT_ROOT/Scripts/cloudlink-guard-ip-risk.jxa" \
...
/usr/bin/grep -Fq '个查询源确认出口一致' "$PROJECT_ROOT/Scripts/cloudlink-guard-check.sh"
/usr/bin/grep -Fq '触发 Cloudflare challenge' "$PROJECT_ROOT/Scripts/cloudlink-guard-check.sh"
/usr/bin/grep -Fq 'TUN DNS 返回 Fake-IP' "$PROJECT_ROOT/Scripts/cloudlink-guard-check.sh"
/usr/bin/grep -Fq '第三方 IP 情报仅供参考' "$PROJECT_ROOT/Scripts/cloudlink-guard-ip-risk.jxa"
/usr/bin/grep -Fq '额外分流链路 ($SECONDARY_LABEL)' "$PROJECT_ROOT/Scripts/cloudlink-guard-check.sh"
/usr/bin/grep -Fq 'https://browserleaks.com/ip' "$PROJECT_ROOT/Scripts/cloudlink-guard-private-browser.sh"
/usr/bin/grep -Fq 'https://iphey.com/' "$PROJECT_ROOT/Scripts/cloudlink-guard-private-browser.sh"
/usr/bin/grep -Fq 'https://scamalytics.com/ip/$EXIT_IP' "$PROJECT_ROOT/Scripts/cloudlink-guard-private-browser.sh"

/usr/bin/grep -q '^prepend-rules:' "$PROJECT_ROOT/Rules/CloudLinkGuard-Merge.yaml"
if /usr/bin/grep -qE '^[[:space:]]*(proxies|proxy-providers):' "$PROJECT_ROOT/Rules/CloudLinkGuard-Merge.yaml"; then
