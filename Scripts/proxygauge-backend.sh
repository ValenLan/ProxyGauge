...
exit_summary() {
  local json summary_ip ip country city location
  ...
  country=$(json_value country_name || true)
  [ -n "$country" ] || country=$(json_value location.country || true)
  [ -n "$country" ] || country=$(json_value country || true)
  [ -n "$country" ] || country=$(json_value cc || true)
  city=$(json_value city || true)
  [ -n "$city" ] || city=$(json_value location.city || true)
  [ -n "$city" ] || city=$(json_value region || true)
  [ -n "$city" ] || city=$(json_value location.region || true)
