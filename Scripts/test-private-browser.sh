...
APP_SOURCE="$PROJECT_ROOT/Sources/PuffRouteApp.swift"

default_output=$(PUFFROUTE_CONFIG=/dev/null \
  PUFFROUTE_PRIVATE_BROWSER_DRY_RUN=1 \
  /bin/bash "$PROJECT_ROOT/Scripts/puffroute-private-browser.sh" default 198.51.100.10)
google_output=$(PUFFROUTE_CONFIG=/dev/null \
  PUFFROUTE_PRIVATE_BROWSER_DRY_RUN=1 \
  /bin/bash "$PROJECT_ROOT/Scripts/puffroute-private-browser.sh" google 203.0.113.20)
...
echo "PuffRoute isolated browser tests passed."
