...
/usr/bin/grep -Fq 'actionLabel: "检查"' "$APP_SOURCE"
...
/usr/bin/grep -Fq 'scopeLabels: ["双出口", "IP 风险", "域名分流"]' "$APP_SOURCE"
/usr/bin/grep -Fq 'scopeLabels.joined(separator: "  ·  ")' "$APP_SOURCE"
/usr/bin/grep -Fq 'detail: isHealthCheckRunning ? "正在检查代理链路…" : "自动检查代理链路"' "$APP_SOURCE"
